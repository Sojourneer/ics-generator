		...
		if (!!nextLine) {
			...
			for (var key in obj) {
				...
				if (typeof val == "object" && !(val instanceof Date)) {
					...
					if (specialType == "VEVENT") {
					...
					} else if (specialType == "VALARM") {
						//attendee object
						var nestedParams = self.filterPropParams("alarm");
						if (key == "alarms") {
							_.each(val, function(alarm) {
								var nextLine = "ALARM";
								newOutput += self.traverseObject(alarm, nestedParams, nextLine);
							});
						}
						...
						throw "IcsGenerator: encountered unexpected object while constructing output string: " + key +
							"; only objects of types: [VEVENT, VALARM, ATTENDEE] are supported at this time."
					}
				} else {
...
IcsUtils.supportedTypesByObject = {
	"calendar": ["prodId", "version", "method", "events"],
	"event": ["summary", "dtStart", "dtEnd", "dtStamp", "organizer", "uid",
				"attendees", "alarms", "created", "description", "location", "sequence",
				"priority", "status", "transp"],
	"attendee": ["cuType", "role", "partStat", "rsvp", "cn", "mailTo"],
	"alarm": ["action","attach","trigger","description"]
}
