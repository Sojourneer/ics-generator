...
IcsEvent = function(options) {
	...
	for (var paramKey in params) {
        if (paramKey == "attendees") {
			this.attendees = !!options.attendees ? _.map(options.attendees, function (event) { return new IcsAttendee(event); }) : _.clone(params.attendees.default);
		} else if (paramKey == "alarms") {
			this.alarms = !!options.alarms ? _.map(options.alarms, function(event) { return new IcsAlarm(event); }) : _.clone(params.alarms.default);
		} else if (paramKey == "organizer") {
			this.organizer = !!options.organizer ? new IcsAttendee(options.organizer) : _.clone(params.organizer.default);
	    } else {
            this[paramKey] = options[paramKey] || _.clone(params[paramKey].default);
        }
...
IcsEvent.prototype = {
	...
	valueOf: function() {
		...
		_.each(keys, function(key) {
			if (key == "attendees") {
				...
				out[key] = _.map(self.alarms, function(alarm) { return alarm.valueOf(); });
	...
	getProperty: function(property) {
		...
		} else if (property == "alarms") {
			return _.map(this.alarms, function(alarm) { return alarm.valueOf(); });
	...
	setProperty: function(property, value) {
        try {
            ...
            if (property == "attendees") {
				...
				this.alarms = _.map(value, function(alarm) { return new IcsAlarm(alarm); });
			} else if (property == "organizer") {
	...
	addAlarm: function(alarm) {
		return this.alarms.push(new IcsAlarm(alarm));
	}
}
